...
class COMradialdistribution:
    ...
    def runradial(self, datfilename, comx, comy, comz, Lx, Ly, Lz, Lx2, Ly2, Lz2, output, nummoltype, moltypel, moltype, firststep=1):
        (maxr, binsize, numbins, count, g) = self.setgparam(Lx2,Ly2,Lz2,firststep,moltypel)  
        while count < len(comx):
            (count) = self.radialdistribution(g, len(comx[1]), moltype, comx, comy, comz, Lx, Ly, Lz, binsize, numbins, maxr, count)
    ...
    def radialnormalization(self,numbins,binsize,Lx,Ly,Lz,nummol,count,g, firststep):
        ...
        for i in range(0,len(g)):
            for j in range(0,len(g)):
                g[i][j] *= Lx*Ly*Lz/nummol[i]/nummol[j]/4/np.pi/(radiuslist)**2/binsize/(count-firststep)
        return (radiuslist)
