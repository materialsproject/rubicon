...
class LmpInput():
    ...
    def __init__(self,mols_in_box,ffmol):
        self.lines = []
    ...
    def _set_top_types(self,ffmol,mols_in_box):
        ...
        for gff,top,v, in zip(ffmol.gff_list,ffmol.top_list,mols_in_box.param_list):
    ...
    def _set_masses(self,ffmol,mols_in_box):
        ...
        for gff,top,mol, parm in zip(ffmol.gff_list,ffmol.top_list,mols_in_box.mols, mols_in_box.param_list):
                ...
                for i, v in enumerate(gff.dihedrals.values()):
                    for func_form, d in v.iteritems():
                        ...
                        lines.append(
                            ...
                            [2],gff.dihedrals.keys()[i][3],mol_index))
            mol_index += 1
    ...
    def _set_atom(self,ffmol,mols_in_box):
        ...
        #print "SELFINDEX",self.box_mol_index
        self.lines.extend(lines)
        return '\n'.join(lines)
        ...
        for gff,top,mol, parm in zip(ffmol.gff_list,ffmol.top_list,mols_in_box.mols, mols_in_box.param_list):
            ...
            for imol in range(num_this_mol):
                ...
                for k, v in enumerate(mol_bonds):
                    ...
                    lines.append('{} {} {} {} {} {} {} {}'.format((i+k+1), bond_type_index[bond_label],
        ...
        for gff,top,mol, parm in zip(ffmol.gff_list,ffmol.top_list,mols_in_box.mols, mols_in_box.param_list):
            ...
            for imol in range(num_this_mol):
                ...
                for k, v in enumerate(top.angles):
                    ...
                    #print "***",angle_type_index, angle_label, a, b , c
                    lines.append('{}  {}  {}  {}  {}  {}  {}  {}  {}  {}'
                    .format(i + k + 1, angle_type_index[angle_label],
                            self.box_mol_index[mol_index-1][v[0]],
                            self.box_mol_index[mol_index-1][v[1]],
                            self.box_mol_index[mol_index-1][v[2]], '#', mol_index,
        ...
        for gff,top,mol, parm in zip(ffmol.gff_list,ffmol.top_list,mols_in_box.mols, mols_in_box.param_list):
            if gff.dihedrals is not None:
                for m, v in enumerate(gff.dihedrals.values()):
                    ...
                    dihedral_type_index[gff.dihedrals.keys()[m]] = dihedral_index
                        ...
                        for func_form, d in v[1].iteritems():
                            ...
                            lines.append(
                                ...
                                .format(j, dihedral_type_index[dihedral_label],
                                        ...
                                        self.box_mol_index[mol_index-1][D], '#', mol_index,
        ...
        for gff,top,mol, parm in zip(ffmol.gff_list,ffmol.top_list,mols_in_box.mols, mols_in_box.param_list):
            if gff.imdihedrals is not None:
                for m, v in enumerate(gff.imdihedrals.values()):
                    ...
                    imdihedral_type_index[gff.imdihedrals.keys()[m]] = imdihedrals_index
                    ...
                    for k, v in enumerate(top.imdihedrals):
                        ...
                        lines.append(
                            ...
                            .format(j, imdihedral_type_index[imdihedral_label],
                                    ...
                                    self.box_mol_index[mol_index-1][v[3]], '#', mol_index,
    ...
    def get_lammps_string(self):
       ...
       my_lammps_list.append(self._set_pair_coeffs(self.ffmol,self.mols_in_box))
       my_lammps_list.append(self._set_bond_coeffs(self.ffmol,self.mols_in_box))
       my_lammps_list.append(self._set_angle_coeffs(self.ffmol,self.mols_in_box))
       my_lammps_list.append(self._set_dihedral_coeffs(self.ffmol,self.mols_in_box))
       my_lammps_list.append(self._set_improper_coeffs(self.ffmol,self.mols_in_box))
       ...
       my_lammps_list.append(self._set_dihedrals(self.ffmol,self.mols_in_box,))
       my_lammps_list.append(self._set_imdihedrals(self.ffmol,self.mols_in_box,))

       return '\n'.join(my_lammps_list)

    def write_lammps(self,filename = None):
