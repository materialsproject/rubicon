...
class BSSEFragment(FWSerializable):
    ...
    def __init__(self, charge, spin_multiplicity, ghost_atoms):
        ...
        for atom in self.ghost_atoms:
            if not isinstance(atom, int):
                raise ValueError(
                    "Each element of ghost atoms must be an integer")
    ...
    def from_dict(cls, m_dict):
        return BSSEFragment(m_dict["charge"], m_dict["spin_multiplicity"],
                            m_dict["ghost_atoms"])


def get_sub_mol(mol, frag):
    species = [site.species_string for i, site in enumerate(mol.sites) if
               i not in frag.ghost_atoms]
    coords = [site.coords for i, site in enumerate(mol.sites) if
              i not in frag.ghost_atoms]
...
def bsse_fws(super_mol_egsnl, name, super_mol_snlgroup_id, super_mol_charge,
             super_mol_spin_multiplicity,
             super_mol_inchi_root, qm_method, fragments, mission,
    ...
    for frag in fragments:
        if len(frag.ghost_atoms) == 0:
            continue
        frag_name = name + "_" + BasisSetSuperpositionErrorCalculationTask.get_fragment_name(
            frag.ghost_atoms)
        fw_ov_creator = QChemFireWorkCreator(mol=super_mol, molname=frag_name,
                                             mission=mission,
        ...
        current_fwid += fwid_incr_factor
        fws_ov = fw_ov_creator.vacuum_only_sp_fw(frag.charge,
                                                 frag.spin_multiplicity,
                                                 fw_ov_cal_id, fw_ov_db_id,
                                                 priority=priority,
                                                 qm_method=qm_method,
                                                 super_mol_snlgroup_id=super_mol_snlgroup_id,
                                                 super_mol_egsnl=super_mol_egsnl,
                                                 super_mol_inchi_root=super_mol_inchi_root,
                                                 ghost_atoms=frag.ghost_atoms,
                                                 bs_overlap=True)
        fws.extend(fws_ov)
        ...
        fw_iso_creator = QChemFireWorkCreator(mol=sub_mol, molname=frag_name,
                                              mission=mission,
                                              dupefinder=dupefinder,
                                              priority=priority,
                                              additional_user_tags=additional_user_tags,
        ...
        current_fwid += fwid_incr_factor
        fws_iso = fw_iso_creator.vacuum_only_sp_fw(frag.charge,
                                                   frag.spin_multiplicity,
                                                   fw_iso_cal_id, fw_iso_db_id,
                                                   priority=priority,
                                                   qm_method=qm_method,
                                                   super_mol_snlgroup_id=super_mol_snlgroup_id,
                                                   super_mol_egsnl=super_mol_egsnl,
    ...
    fw_bsse = Firework([BasisSetSuperpositionErrorCalculationTask()],
                       spec=bsse_spec, name=name + " BSSE Calculation",
                       fw_id=current_fwid)
    for i in db_fwids:
