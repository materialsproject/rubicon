...
def single_point_energy_fws(mol, name, mission, solvent, solvent_method,
                            use_vdW_surface, qm_method, pop_method,
                            dupefinder=None, priority=1,
                            parent_fwid=None, additional_user_tags=None,
                            depend_on_parent_fw=True, large=None):
    if large is None:
        large = False
        if len(mol) > 50:
            large = True
    ...
    if len(mol) > 1:
        ...
        fw_geom = fw_creator.geom_fw(
            charge, spin_multiplicity, geom_cal_fwid, geom_db_fwid, priority,
            geom_method)
        fws.extend(fw_geom)
        ...
        fw_freq = fw_creator.freq_fw(
            charge, spin_multiplicity, freq_cal_fwid, freq_db_fwid, priority,
            geom_method)
        fws.extend(fw_freq)
    ...
    fw_sol = fw_creator.sp_fw(
        charge, spin_multiplicity, sol_cal_fwid, sol_db_fwid,
        solvent_method=solvent_method,
        use_vdw_surface=use_vdW_surface, solvent=solvent,
        priority=priority, qm_method=sol_qm_method,
        population_method=pop_method, task_type_name="solvation energy")
    fws.extend(fw_sol)
    ...
    fw_vac_sp = fw_creator.vacuum_only_sp_fw(
        charge, spin_multiplicity, vac_sp_cal_fwid, vac_sp_db_fwid,
        priority=priority, qm_method=energy_method)
