...
class IonPair:
    ...
    Takes in the center of mass coordinates for all molecules and
    timesteps from calcCOM as well as other properties already parsed.
        
    The correlation function is fit to a single, double, triple,
    quadrouple and quintuple exponential, to find the best fit without
    overfitting the data.
        
    output includes the correlation function, the ionpair lifetimes from
    the different fits as well as the r-squared values for the
    different fits
    ...
    def runionpair(self, comx, comy, comz, Lx, Ly, Lz, moltypel, moltype,
        ...
        output['Ion_Pair_Lifetime'][
            'Explanation'] = "The Ion Pair Lifetime correlation function is " \
                             "fit to a single exponential, a double " \
                             "exponential up to 5 exponentials. The results " \
                             "shown are the result of these successive fittings"
        (closest, begin, end, C) = self.init(len(comx[0]), moltypel, len(comx),
                                             moltype)
        for step in range(0, len(comx)):
            r = self.calcdistance(comx[step], comy[step], comz[step], Lx, Ly,
                                  Lz)
    ...
    def exponential3(self, x, A1, A2, A3, B1, B2, B3):
        return A1 * np.exp(-x / B1) + A2 * np.exp(-x / B2) + A3 * np.exp(
            -x / B3)
            ...
            for fun in funlist:
                try:
                    popt, pcov = curve_fit(fun, np.array(time[begin:end]),
                                           np.array(correlation[begin:end]),
                                           maxfev=1000000)
                except:
