...
log file: contains the thermo dynamic data with the format set by the
...
class LammpsRun(object):
    ...
    """
    def __init__(self, data_file, trajectory_file, log_file="log.lammps", is_forcefield=False):
    ...
    def _parse_trajectory(self):
        ...
        # "ITEM: ATOMS id type ...
        traj_label_pattern = re.compile("^\s*ITEM:\s+ATOMS\s+id\s+type\s+([A-Za-z\s]*)")
        ...
        with open(self.trajectory_file) as tf:
            for line in tf:
                ...
                if traj_label_pattern.search(line):
                    ...
                    traj_pattern_string = "\s*(\d+)\s+(\d+)"+"".join(["\s+([0-9eE\.+-]+)"
                ...
                if traj_pattern.search(line):
                    ...
                    line_data.extend([float(x) for i, x in enumerate(m.groups()) if i+1 > 2])
    ...
    def get_structures_from_trajectory(self):
        ...
        """
        structures = []
        mass_to_symbol = dict((round(y["Atomic mass"], 1), x) for x, y in _pt_data.items())
        unique_atomic_masses = np.array(self.lammps_data.atomic_masses)[:, 1]
        for step in range(self.timesteps.size):
            begin = step * self.natoms
            end = (step + 1) * self.natoms
            ...
            try:
                ...
                print("Error: '{}' at timestep {} in the trajectory".format(error,
    ...
    def get_displacements(self):
        ...
        for step in range(self.timesteps.size):
            ...
            step_frac_coords = [lattice.get_fractional_coords(crd) for crd in coords]
    ...
    def get_diffusion_analyzer(self, specie, temperature, time_step, step_skip, smoothed=None,
        ...
        return DiffusionAnalyzer(structure, disp, specie, temperature, time_step,
                                 step_skip=step_skip, smoothed=smoothed, min_obs=min_obs,
        ...
        with open(self.log_file, 'r') as logfile:
            for line in logfile:
                ...
                if format and not thermo_data:
                    ...
                    thermo_pattern_string = "\s*([0-9eE\.+-]+)" + "".join(["\s+([0-9eE\.+-]+)"
                                                                        for _ in
                                                                        range(len(fields)-1)])
                ...
                if thermo_pattern:
                    if thermo_pattern.search(line):
                        ...
                        thermo_data.append(tuple([float(x) for i, x in enumerate(m.groups())]))
        thermo_data_dtype = np.dtype([(fld.encode("ascii"), np.float64) for fld in fields])
