...
class TestLammpsOutput(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.lammpsrun = LammpsRun(data_file, traj_file, log_file, is_forcefield=True)
    ...
    def test_lammps_log(self):
        ...
        fields = fields.split()
        thermo_data_ans = np.loadtxt(os.path.join(module_dir, "test_files", "thermo_data.txt"))
        thermo_data = self.lammpsrun.lammps_log.thermo_data
        self.assertEqual(list(thermo_data.dtype.names), fields)
        self.assertEqual(self.lammpsrun.lammps_log.nmdsteps+1, thermo_data.shape[0])
        thermo_data = thermo_data[:].view(np.float64).reshape(thermo_data.shape + (-1,))
        np.testing.assert_almost_equal(thermo_data, thermo_data_ans, decimal=10)
    ...
    def test_lammps_trajectory(self):
        ...
        np.testing.assert_almost_equal(trajectory[:][fields[0]], trajectory_ans[:, 0]-1, decimal=10)
        for i, fld in enumerate(fields[1:]):
            np.testing.assert_almost_equal(trajectory[:][fld], trajectory_ans[:, i+1], decimal=10)
