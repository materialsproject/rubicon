...
connection = pymongo.Connection('mongodb://:@mongodb03.nersc.gov:27017/')
...
for formula in formula_list:
    ...
    cursor = materials.find(
        {"pretty_formula": formula[0] + "2" + formula[1] + "O4"}, {"_id": 0})
    if not cursor.count():
        cursor = materials.find(
            {"pretty_formula": formula[1] + formula[0] + "2O4"}, {"_id": 0})
        if not cursor.count():
            cursor = materials.find(
                {"pretty_formula": formula[1] + "(" + formula[0] + "O2)2"},
                {"_id": 0})
    counter = 1
    dcl = []
    for c in cursor:
